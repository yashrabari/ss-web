...
import { post2fa, postResetPasswordResend, postResetPasswordVerify } from "../../api"
import { useState } from "react"
import { useNavigate } from "react-router-dom"
import { passwordReseted, requestResetPassword, setResetPasswordError } from "../../store/slice/mainSlice"
import { useSetNewPasswordMutation, useVerifyOtpMutation } from "../../store/slice/api"
export default function VerifyCode({ twofactor, imageSrc, title }) {
  ...
  const handleVerifyOtp = () => {
    if (twofactor) {
      ...
      verifyOtp({code, email}).unwrap().then((response) => {
        // changes 
        if (response.message) {
  ...
  return (
    <Page>
      ...
      >
        ...
        <ButtonBar>
          <Button color="#00A652" onClick={handleVerifyOtp}>Submit</Button>
