...
import { Row, Title, IconButton, Paragraph, Button, OptionsMenu } from '../../components/common'
...
import { useDeleteCardMutation, useGetAllCardsQuery, useMarkAsPrimaryMutation } from '../../store/slice/api'
...
const Transactions = () => {
    ...
    return (
        <>
            ...
            >
                <BackTransactions />
                <Title fontWeight='650' margin='0px 0px 0px 80px'>My Transactions</Title>
                <Row>
                    <OptionMenuSettings
                        options={[
            ...
            >
                ...
                <Button
                    ...
                    height="46px"
                    color="#00A652"
                    onClick={() => { navigate('/home/transactions/addPaymentMethod') }}
                >
                    + Add Payment Method
                </Button>
            </Row>
            <Table>
                <tbody>
                    {allCards && allCards.data && allCards.data.map((item, index) => (
                        <TableBodyRow key={index}>
                            <td>
                                ...
                                >
                                    ...
                                    {item.metadata && item.metadata.default === 'True' && <span style={{marginLeft: '20px', border: '1px solid #4aa25c', color: '#4aa25c', fontFamily: 'TT Commons', padding: '2px 5px', borderRadius: '5px'}} >Primary</span> }
                                </Grid>
                            </td>
                            <td>
                                <Grid item lg={9}
                            ...
                            <td>
                                <Grid item lg={0.5}>
                                    <OptionsMenu
                                        options={[
                                            { text: "Mark as primary", onClick: () => { markAsPrimary(item.id) } },
                                            { text: "Delete", onClick: () => { deleteCard(item.id) } }
                                        ]}
                                    />
                                </Grid>
                            </td>
            ...
            >
                <Button
                    ...
                    height="46px"
                    color="#00A652"
                    onClick={() => { navigate('/home/transactions/payment-history') }}
                >
