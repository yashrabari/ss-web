...
const Index = () => {
  ...
  const handleSubmit = () => {
    if (!title || !message) return alert('please enter title and message')
      ...
      sendFeedback({ title: title, message: message }).unwrap().then((resp) => {
        setTitle('')
        setMessage('')
    ...
    <>
      ...
      >
        <Row alignItems="center">
          ...
          <Title fontWeight='650' margin='0px 0px 0px 40px'>Send Feedback</Title>
        </Row>
        <Row>
          <OptionMenuSettings
            options={[
      ...
      <div style={{ padding: '0px 40px' }}>
        <Row
          ...
          <Title fontWeight='650' fontSize='20px'>{user.email}</Title>
        </Row>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', width: '849px', marginTop: '10px' }}>
          <Button width='180px' onClick={() => handleSubmit()} color="#00A652" type="submit">
            Send
          </Button>
        </div>
      </div>
