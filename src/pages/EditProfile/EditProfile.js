...
import { useGetCountriesQuery, useUpdateProfileMutation } from "../../store/slice/api";
...
export default function EditProfile() {
    ...
    useEffect(() => {
        getUserProfile(token).then((res) => {
            ...
            if (data) {
                if (fullName != data.name || userName != data.username || profilePic != data.profile_picture || location != data.location || verificationPeriod != data.verification_period) {
    ...
    const handleImage = e => {
        ...
        if (file.type.split('/')[0] !== 'image') return alert("This file type is not supported")
    ...
    const onSubmit = e => {
        ...
        let dataToSubmit = { id: userID, user_data: JSON.stringify({ name: fullName, username: userName, phone_number: phoneNumber, email, location:country, verification_period: verificationPeriod.name ? verificationPeriod.name : verificationPeriod }) }
    ...
    return (
        <>
            ...
            >
                            ...
                            <div key={index}>
                                ...
                                <div style={{ display: 'block', marginTop: '1rem', marginBottom: '1rem' }}>
                                    ...
                                    <Select
                                        ...
                                        value={verificationperiodoptions.find(el => el.value === verificationPeriod)}
                                        styles={customStyles}
                                        onChange={(e) => {
                                            console.log(e)
                ...
                <LogoContainer backgroundColor='#ebebeb6b' onClick={() => profilePicRef.current.click()}>
