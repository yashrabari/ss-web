...
import { Button, IconButton, InputGroup, OptionsMenu, Paragraph, Row, Title } from "../../../../components/common"
...
import { useCreateFolderMutation, useDeleteFolderMutation, useGetFoldersQuery } from "../../../../store/slice/api"
...
export default function Documents() {
  ...
  return (
    <>
      ...
      >
        <BackTransactions />
        <Title margin='0px 0px 0px 80px'>Uploaded Folders</Title>
          ...
          <Row height='100vh'>
            <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%', width: '100%' }}>
              <div style={{ width: '160px', height: '160px' }}>
                <img style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'center' }} src={File} />
              </div>
              <div>
                <h1 style={{ fontFamily: 'TT Commons', fontWeight: 400, fontSize: '24px' }}>Uploaded file will be displayed here</h1>
              </div>
            </div>
          </Row>
        </> :
              ...
              {allFolders?.map((item, index) => {
                return (
                  <div style={{ position: 'relative' }}>
                      <div onClick={() => navigate(`/home/documents/folder/${item.id}`)}>
                        <FolderContainer
                          key={index}
                          width='274px'
                          height='173px'
                        ...
                        >
                          ...
                          <Title fontSize='22px' margin='20px 0px 0px 3px' lineHeight='38px' fontWeight='600' fontFamily='TT Commons'>{item.name}</Title>                          
                          <Paragraph fontSize='16px' margin='10px 0px 0px 3px'>{item.files.length} file</Paragraph>                        </FolderContainer>
                      </div>
                      <OptionsMenu
                        color="rgba(0, 0, 0, 0.4)"
                        orientation="horizontal"
                        options={[
                          { text: "Open", onClick: () => { navigate(`/home/documents/folder/${item.id}`) } },
