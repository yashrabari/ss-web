...
const ShareWith = ({ details }) => {
  ...
  const buddies = useMemo(() => {
    ...
    return myBuddies
      .filter(
        (bud) =>
          !Boolean(details?.sharedWith?.length) ||
          details?.sharedWith.every((user) => user.id !== bud.id)
      )
      .map((bud) => ({
        value: bud.id,
        label: bud.buddydetail.username,
        image: bud.buddydetail.profile_picture,
      }));
  }, [myBuddies, details.sharedWith]);

  const filteredBuddies = useMemo(() => {
    if (!Boolean(selectedBuddies.length)) return buddies;
    return buddies.filter((bud) =>
      selectedBuddies.every((selected) => bud.value !== selected.value)
    );
